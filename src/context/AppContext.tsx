...
type Action =
  ...
  | { type: 'TOGGLE_SIDEBAR' }
  | { type: 'TOGGLE_MAINTENANCE'; payload: { buildingId: string; generateReceipt: boolean } }
  | { type: 'REPORT_FAULT'; payload: { buildingId: string; reporterName: string; reporterSurname: string; reporterPhone: string; apartmentNo: string; description: string } }
...
const appReducer = (state: AppState, action: Action): AppState => {
  switch (action.type) {
    ...
    case 'TOGGLE_MAINTENANCE':
      {
        const { buildingId, generateReceipt } = action.payload;
        const building = state.buildings.find(b => b.id === buildingId);
        if (!building) return state;

        const isMaintained = !building.isMaintained;
        const now = new Date();
        const maintenanceDate = now.toISOString().split('T')[0];
        const maintenanceTime = now.toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' });
        const performedBy = state.currentUser?.name || 'Bilinmeyen Teknisyen';
        const totalFee = building.maintenanceFee * building.elevatorCount;
        ...
        const newMaintenanceRecord: MaintenanceRecord = {
          ...
          status: 'completed',
          priority: 'medium',
          searchableText: `${building.name} ${building.address.mahalle} ${building.address.sokak} ${performedBy}`.toLowerCase(),
        };

        let updatedDebtRecords = [...state.debtRecords];
        let updatedBuildings = state.buildings.map(b => {
          if (b.id === buildingId) {
            const newDebt = b.debt + totalFee;
            const newDebtRecord: DebtRecord = {
              id: uuidv4(),
              buildingId: b.id,
              date: now.toISOString(),
              type: 'maintenance',
              description: `${b.name} için ${maintenanceDate} tarihli bakım ücreti`,
              amount: totalFee,
              previousDebt: b.debt,
              newDebt: newDebt,
              performedBy: performedBy,
              relatedRecordId: newMaintenanceRecord.id,
            };
            updatedDebtRecords.push(newDebtRecord);

            return {
              ...b,
              isMaintained: isMaintained,
              lastMaintenanceDate: isMaintained ? maintenanceDate : undefined,
              lastMaintenanceTime: isMaintained ? maintenanceTime : undefined,
              debt: newDebt,
              isDefective: false,
        ...
        let newArchivedReceipt: ArchivedReceipt | undefined;
        if (generateReceipt) {
          const receiptHtml = generateMaintenanceReceipt(building, state, performedBy);
          newArchivedReceipt = {
            id: uuidv4(),
        ...
        const notificationMessage = `${building.name} binasının bakımı ${isMaintained ? 'tamamlandı' : 'geri alındı'}.`;
        const updatedNotifications = [...state.notifications, notificationMessage];
        const updatedUnreadNotifications = state.unreadNotifications + 1;
        ...
        return {
          ...
          archivedReceipts: newArchivedReceipt ? [...state.archivedReceipts, newArchivedReceipt] : state.archivedReceipts,
          showReceiptModal: generateReceipt,
          receiptModalHtml: generateReceipt ? newArchivedReceipt?.htmlContent || null : null,
        ...
        const updatedBuildings = state.buildings.map(b =>
          b.id === buildingId
            ? {
                ...
                faultReportedBy: `${reporterName} ${reporterSurname}`,
              }
            : b
        );

        const buildingName = updatedBuildings.find(b => b.id === buildingId)?.name || 'Bilinmeyen Bina';
        const notificationMessage = `${buildingName} için yeni arıza bildirimi (${reporterName} ${reporterSurname} tarafından).`;
        const updatedNotifications = [...state.notifications, notificationMessage];
        const updatedUnreadNotifications = state.unreadNotifications + 1;
    ...
    case 'ADD_PAYMENT':
      {
        ...
        const updatedBuildings = state.buildings.map(b => {
          if (b.id === buildingId) {
            const newDebt = b.debt - amount;
            const newDebtRecord: DebtRecord = {
              id: uuidv4(),
              buildingId: b.id,
              date: new Date().toISOString(),
              type: 'payment',
              description: `${b.name} için ${amount.toLocaleString('tr-TR')} ₺ ödeme alındı.`,
              amount: amount,
              previousDebt: b.debt,
              newDebt: newDebt,
            ...
            updatedDebtRecords.push(newDebtRecord);
            return { ...b, debt: newDebt };
          }
          return b;
        });

        const buildingName = updatedBuildings.find(b => b.id === buildingId)?.name || 'Bilinmeyen Bina';
        const notificationMessage = `${buildingName} için ${amount.toLocaleString('tr-TR')} ₺ ödeme alındı.`;
        const updatedNotifications = [...state.notifications, notificationMessage];
        const updatedUnreadNotifications = state.unreadNotifications + 1;
...
interface AppContextType {
  ...
  toggleSidebar: () => void;
  toggleMaintenance: (buildingId: string, generateReceipt?: boolean) => void;
  reportFault: (buildingId: string, faultData: { reporterName: string; reporterSurname: string; reporterPhone: string; apartmentNo: string; description: string }) => void;
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const toggleMaintenance = (buildingId: string, generateReceipt: boolean = false) => {
    dispatch({ type: 'TOGGLE_MAINTENANCE', payload: { buildingId, generateReceipt } });
  };

  const reportFault = (buildingId: string, faultData: { reporterName: string; reporterSurname: string; reporterPhone: string; apartmentNo: string; description: string }) => {
