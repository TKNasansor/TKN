...
// Firebase imports
// Bu import'lar, Canvas ortamında Firebase SDK'nın yüklü olduğunu varsayar.
// Eğer "Failed to resolve import 'firebase/app'" gibi bir hata alırsanız,
// projenizin terminalinde 'npm install firebase' komutunu çalıştırmanız gerekebilir.
...
const initialState: AppState = {
  ...
  updates: [],
  incomes: [],
  payments: [], // Ödeme takibi için eklendi
  currentUser: null,
  users: [],
  notifications: [],
  ...
  settings: {
    ...
    ceEmblemUrl: '/ce.png', // Varsayılan değer
    tseEmblemUrl: '/ts.jpg', // Varsayılan değer
    receiptTemplate: `
  ...
  proposalTemplates: [],
  qrCodes: [],
  systemNotifications: [], // Sistem bildirimleri için yeni alan
  autoSaveData: [],
...
// AppContextType tanımı: Tüm context değerlerini ve fonksiyonlarını içerir
interface AppContextType {
  ...
  addNotification: (notification: AppNotificationData) => Promise<void>; // Firebase ile uyumlu
  clearNotifications: () => Promise<void>; // Firebase ile uyumlu
  toggleSidebar: () => void;
  toggleMaintenance: (buildingId: string, showReceipt?: boolean) => void;
  reportFault: (buildingId: string, faultData: { description: string; severity: 'low' | 'medium' | 'high'; reportedBy: string }) => Promise<void>; // Firebase ile uyumlu
  updateSettings: (settings: Partial<AppState['settings']>) => void; // Daha spesifik tip
  resetMaintenanceStatus: () => void;
  addFaultReport: (buildingId: string, reporterName: string, reporterSurname: string, reporterPhone: string, apartmentNo: string, description: string) => Promise<void>; // Firebase ile uyumlu
  resolveFaultReport: (id: string) => void;
  addMaintenanceHistory: (history: Omit<MaintenanceHistory, 'id'>) => void;
  addMaintenanceRecord: (record: Omit<MaintenanceRecord, 'id'>) => void;
  addPrinter: (printer: Omit<Printer, 'id'>) => void; // Daha spesifik tip
  updatePrinter: (printer: Printer) => void; // Daha spesifik tip
  deletePrinter: (id: string) => void;
  addSMSTemplate: (template: Omit<SMSTemplate, 'id'>) => void; // Daha spesifik tip
  updateSMSTemplate: (template: SMSTemplate) => void; // Daha spesifik tip
  ...
  addProposal: (proposal: Omit<Proposal, 'id' | 'createdDate' | 'createdBy'>) => void; // Daha spesifik tip
  updateProposal: (proposal: Proposal) => void; // Daha spesifik tip
  deleteProposal: (id: string) => void;
  addPayment: (payment: Omit<Payment, 'id'>) => void; // Daha spesifik tip
  addProposalTemplate: (template: Omit<ProposalTemplate, 'id'>) => void; // Daha spesifik tip
  updateProposalTemplate: (template: ProposalTemplate) => void; // Daha spesifik tip
  deleteProposalTemplate: (id: string) => void;
  addQRCodeData: (qrData: Omit<QRCodeData, 'id'>) => void; // Daha spesifik tip
  updateAutoSaveData: (data: AutoSaveData) => void; // Daha spesifik tip
  ...
  addSystemNotification: (notification: AppNotificationData) => void; // Sistem bildirimi fonksiyonu
}

const AppContext = createContext<AppContextType | undefined>(undefined);

// AppAction tipleri: Reducer tarafından işlenecek tüm olası eylemleri tanımlar
type AppAction =
  | { type: 'ADD_BUILDING'; payload: Omit<Building, 'id'> } // Payload Omit ile güncellendi
  | { type: 'UPDATE_BUILDING'; payload: Building }
  | { type: 'DELETE_BUILDING'; payload: string }
  | { type: 'ADD_PART'; payload: Omit<Part, 'id'> } // Payload Omit ile güncellendi
  | { type: 'UPDATE_PART'; payload: Part }
  | { type: 'DELETE_PART'; payload: string }
  | { type: 'INSTALL_PART'; payload: Omit<PartInstallation, 'id' | 'installedBy'> } // Payload Omit ile güncellendi
  | { type: 'INSTALL_MANUAL_PART'; payload: Omit<ManualPartInstallation, 'id' | 'installedBy'> } // Payload Omit ile güncellendi
  | { type: 'MARK_PART_AS_PAID'; payload: { installationId: string; isManual: boolean } } // partId -> installationId
  | { type: 'ADD_UPDATE'; payload: Omit<Update, 'id' | 'timestamp'> } // Payload Omit ile güncellendi
  | { type: 'ADD_INCOME'; payload: Omit<Income, 'id'> } // Payload Omit ile güncellendi
  | { type: 'SET_USER'; payload: string } // Payload User objesi yerine string olarak güncellendi (fonksiyona göre)
  | { type: 'DELETE_USER'; payload: string }
  | { type: 'ADD_NOTIFICATION_LOCAL'; payload: AppNotificationData } // Firebase'den önce lokal bildirimler için
  | { type: 'SET_NOTIFICATIONS_FROM_DB'; payload: AppNotificationData[] } // Firebase'den bildirim yüklemek için
  | { type: 'CLEAR_NOTIFICATIONS' }
  | { type: 'TOGGLE_SIDEBAR' }
  | { type: 'TOGGLE_MAINTENANCE'; payload: { buildingId: string; showReceipt: boolean } }
  | { type: 'REPORT_FAULT'; payload: { buildingId: string; faultData: { description: string; severity: 'low' | 'medium' | 'high'; reportedBy: string } } } // Daha spesifik tip
  | { type: 'UPDATE_SETTINGS'; payload: Partial<AppState['settings']> } // Daha spesifik tip
  | { type: 'RESET_MAINTENANCE_STATUS' }
  | { type: 'ADD_FAULT_REPORT'; payload: Omit<FaultReport, 'id' | 'timestamp' | 'status'> } // Payload Omit ile güncellendi
  ...
  | { type: 'ADD_PRINTER'; payload: Omit<Printer, 'id'> } // Payload Omit ile güncellendi
  | { type: 'UPDATE_PRINTER'; payload: Printer }
  | { type: 'DELETE_PRINTER'; payload: string }
  | { type: 'ADD_SMS_TEMPLATE'; payload: Omit<SMSTemplate, 'id'> } // Payload Omit ile güncellendi
  ...
  | { type: 'ADD_PROPOSAL'; payload: Omit<Proposal, 'id' | 'createdDate' | 'createdBy'> } // Payload Omit ile güncellendi
  | { type: 'UPDATE_PROPOSAL'; payload: Proposal }
  | { type: 'DELETE_PROPOSAL'; payload: string }
  | { type: 'ADD_PAYMENT'; payload: Omit<Payment, 'id'> } // Payload Omit ile güncellendi
  | { type: 'ADD_PROPOSAL_TEMPLATE'; payload: Omit<ProposalTemplate, 'id'> } // Payload Omit ile güncellendi
  | { type: 'UPDATE_PROPOSAL_TEMPLATE'; payload: ProposalTemplate }
  | { type: 'DELETE_PROPOSAL_TEMPLATE'; payload: string }
  | { type: 'ADD_QR_CODE_DATA'; payload: Omit<QRCodeData, 'id'> } // Payload Omit ile güncellendi
  | { type: 'UPDATE_AUTO_SAVE_DATA'; payload: AutoSaveData } // Daha spesifik tip
  | { type: 'SHOW_RECEIPT_MODAL'; payload: string }
  | { type: 'CLOSE_RECEIPT_MODAL' }
  | { type: 'ARCHIVE_RECEIPT'; payload: Omit<ArchivedReceipt, 'id'> } // Payload Omit ile güncellendi
  ...
  | { type: 'ADD_SYSTEM_NOTIFICATION'; payload: AppNotificationData }; // Sistem bildirimi aksiyonu

// Reducer fonksiyonu: State'i eylemlere göre günceller
function appReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case 'ADD_NOTIFICATION_LOCAL':
      return {
        ...state,
        notifications: [action.payload.message, ...state.notifications],
        unreadNotifications: state.unreadNotifications + 1,
      };

    case 'SET_NOTIFICATIONS_FROM_DB':
        const notificationsFromDb = action.payload.map(n => n.message);
        return {
            ...state,
            notifications: notificationsFromDb,
            unreadNotifications: notificationsFromDb.length,
        };
    ...
    case 'ADD_SYSTEM_NOTIFICATION':
      return {
        ...state,
        systemNotifications: [...state.systemNotifications, action.payload],
      };
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const authReady = useRef(false); // Auth durumunun kontrol edildiğini takip etmek için

  // Sistem bildirimi gönderme fonksiyonu
  const addSystemNotification = (notification: AppNotificationData) => {
...
@@ .. @@
  ...
  const sendBulkSMS = (templateId: string, buildingIds: string[]) => {
    ...
    // Bu kısım UI'da bir bildirim olarak gösterilebilir
    addNotification({
      id: uuidv4(),
      message: `${buildingIds.length} binaya SMS gönderildi`,
      timestamp: new Date().toISOString(),
      type: 'info',
      severity: 'low',
      actionRequired: false,
  ...
  const sendWhatsApp = (templateId: string, buildingIds: string[]) => {
    ...
    addNotification({
      id: uuidv4(),
      message: `${buildingIds.length} bina için WhatsApp pencereleri açıldı`,
      timestamp: new Date().toISOString(),
      type: 'info',
      severity: 'low',
      actionRequired: false,
  ...
  // Bu fonksiyon getLatestArchivedReceiptHtmlMemoized olarak dışa aktarılacak
