...
type Action =
  ...
  | { type: 'TOGGLE_SIDEBAR' }
  | { type: 'MARK_MAINTENANCE_DONE'; payload: { buildingId: string; showReceipt: boolean } }
  ...
  | { type: 'VIEW_ARCHIVED_RECEIPT'; payload: string }
  | { type: 'UNDO_MAINTENANCE'; payload: string };
...
function appReducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case 'UNDO_MAINTENANCE': // Yeni aksiyon
      ...
      const lastMaintenanceHistoryEntryIndex = newMaintenanceHistoryAfterUndo
      ...
      if (lastMaintenanceHistoryEntryIndex.length > 0) {
        const lastHistoryId = lastMaintenanceHistoryEntryIndex[0].id;
        newMaintenanceHistoryAfterUndo = newMaintenanceHistoryAfterUndo.filter(mh => mh.id !== lastHistoryId);
      }

      // En son arşivlenmiş makbuzu kaldır
      const lastArchivedReceiptEntryIndex = newArchivedReceiptsAfterUndo
          .filter(ar => ar.buildingId === action.payload)
          .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
      
      if (lastArchivedReceiptEntryIndex.length > 0) {
        const lastReceiptId = lastArchivedReceiptEntryIndex[0].id;
        newArchivedReceiptsAfterUndo = newArchivedReceiptsAfterUndo.filter(ar => ar.id !== lastReceiptId);
      }

      // En son bakım kaydını kaldır
      const lastMaintenanceRecordEntryIndex = newMaintenanceRecordsAfterUndo
          .filter(mr => mr.buildingId === action.payload)
          .sort((a, b) => new Date(b.maintenanceDate + ' ' + b.maintenanceTime).getTime() - new Date(a.maintenanceDate + ' ' + a.maintenanceTime).getTime());
      
      if (lastMaintenanceRecordEntryIndex.length > 0) {
        const lastRecordId = lastMaintenanceRecordEntryIndex[0].id;
        newMaintenanceRecordsAfterUndo = newMaintenanceRecordsAfterUndo.filter(mr => mr.id !== lastRecordId);
      }
